...
1. Click **Choose File** and select a `.px` file exported from another ProxMigrate instance
...
- **Step 3 — Deploy**: ProxMigrate transfers the backup to Proxmox and restores the container

## Requirements

- The file must be a `.px` package exported from ProxMigrate's container export feature
...
**"Invalid package" error** — the file is not a valid ProxMigrate container export. Ensure it was exported from a ProxMigrate instance (not a raw vzdump backup).
