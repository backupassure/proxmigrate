...
ProxMigrate uses SSH key authentication for all ongoing communication with your Proxmox server. This step copies the ProxMigrate public key to Proxmox so future operations don't require a password.
...
- **Auditability** — SSH key auth is logged by Proxmox's `sshd`, making it easy to see when ProxMigrate connected.

## What the public key looks like

The text displayed in the "ProxMigrate Public Key" box is your instance's public key. It begins with `ssh-rsa` or `ssh-ed25519` followed by a long base64 string and ends with an identifier. This key is safe to share — it cannot be used to derive the private key.
...
**"Host key verification failed"** — The Proxmox host's fingerprint changed (possible if you reinstalled Proxmox). If you're sure the host is legitimate, clear the known_hosts entry on the ProxMigrate server.

**Key already exists** — If you're re-running the wizard, the key may already be in authorized_keys. ProxMigrate checks for duplicates and won't add it twice.
