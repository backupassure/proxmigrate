...
**Proxmox Host** — The IP address or hostname of your Proxmox VE server. This must be reachable from the server running ProxMigrate. Examples:
- `192.168.1.100`
...
- The ProxMigrate server must be able to reach your Proxmox host on both ports
- If using a hostname, DNS must resolve correctly from the ProxMigrate server
...
**"Network unreachable" or timeout** — The ProxMigrate server can't reach the Proxmox host. Check routing, firewalls, and VLANs between the two servers.

**Hostname not resolving** — Try using an IP address instead. Or check that DNS is properly configured on the ProxMigrate server (`/etc/resolv.conf`).
